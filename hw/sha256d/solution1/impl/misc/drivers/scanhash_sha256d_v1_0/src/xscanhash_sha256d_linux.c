...
typedef struct {
    ...
    XScanhash_sha256d_uio_map maps[ MAX_UIO_MAPS ];
} XScanhash_sha256d_uio_info;

/***************** Variable Definitions **************************************/
static XScanhash_sha256d_uio_info uio_info;
...
static int uio_info_read_name(XScanhash_sha256d_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XScanhash_sha256d_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XScanhash_sha256d_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XScanhash_sha256d_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XScanhash_sha256d_Initialize(XScanhash_sha256d *InstancePtr, const char* InstanceName) {
	XScanhash_sha256d_uio_info *InfoPtr = &uio_info;
